	...
	public function executePostActions(\DataContainer $dc)
	{
		...
		switch ($this->strAction)
		{
			...
			case 'toggleSubpalette':
				...
				if (!isset($GLOBALS['TL_DCA'][$dc->table]['palettes']['__selector__']) || !is_array($GLOBALS['TL_DCA'][$dc->table]['palettes']['__selector__']) || !in_array(\Input::post('field'), $GLOBALS['TL_DCA'][$dc->table]['palettes']['__selector__']) || (isset($GLOBALS['TL_DCA'][$dc->table]['fields'][\Input::post('field')]['exclude']) && $GLOBALS['TL_DCA'][$dc->table]['fields'][\Input::post('field')]['exclude'] && !$this->User->hasAccess($dc->table . '::' . \Input::post('field'), 'alexf')))
				{
					$this->log('Field "' . \Input::post('field') . '" is not an allowed selector field (possible SQL injection attempt)', 'Ajax executePostActions()', TL_ERROR);
